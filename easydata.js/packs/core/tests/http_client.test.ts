...
// Note: latte's toHaveBeenCalledWith() takes the expected arguments as one array.
...
describe('HttpClient', () => {
    ...
    let originalWindowXMLHttpRequest: any;

    // latte has no global fail()
    ...
    beforeEach(() => {
        ...
        xhrMock = {
            open: mock(),
            // The response is "received" as soon as the request is sent
            send: mock(() => { xhrMock.readyState = 4; }),
            setRequestHeader: mock(),
            // HttpRequest.open() only opens a request that is not sent yet
            readyState: 0,
            UNSENT: 0,
            status: 200,
            responseText: '{"success":true,"data":"test"}',
            getResponseHeader: mock(() => 'application/json'),
            onreadystatechange: null,
            onerror: null
        };

        // Save original XMLHttpRequest and replace it with mock. HttpClient uses the
        // global XMLHttpRequest, which under latte is not window.XMLHttpRequest
        originalXMLHttpRequest = (globalThis as any).XMLHttpRequest;
        originalWindowXMLHttpRequest = window.XMLHttpRequest;
        const xhrConstructor = mock().mockImplementation(() => {
            const xhr = xhrMock;
            requests.push(xhr);
            return xhr;
        });
        (globalThis as any).XMLHttpRequest = xhrConstructor;
        (window as any).XMLHttpRequest = xhrConstructor;
    ...
    afterEach(() => {
        ...
        (window as any).XMLHttpRequest = originalWindowXMLHttpRequest;
    });
    ...
    it('should perform DELETE request', () => {
        ...
        expect(xhrMock.send.mock.calls[0][0]).toBeUndefined();
    });
    ...
    it('should handle responses with different data types', async () => {
        ...
        xhrMock.getResponseHeader.mockImplementation(() => 'text/plain');
